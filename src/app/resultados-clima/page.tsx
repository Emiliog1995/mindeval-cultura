...
function ResultadosClimaContent() {
  ...
  async function descargarPDF() {
    ...
    const scores: ClimaResult   = rec.scores as ClimaResult;
    ...
    doc.text("Assessment Center Digital — Encuesta de Clima Laboral", 14, 22);
    ...
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.text("DATOS DEL PARTICIPANTE", 14, 46);
    autoTable(doc, {
      startY: 50,
      head: [],
      body: [
        ...
        ["Área",    rec.area    ?? "—"],
        ["Empresa", rec.empresa ?? "—"],
        ["Fecha",   new Date(rec.created_at).toLocaleDateString("es-EC")],
      ],
      theme: "grid",
    ...
    doc.setFontSize(11);
    doc.setTextColor(26, 32, 53);
    doc.text("INTERPRETACIÓN", 14, afterDims);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    const lineas = doc.splitTextToSize(interpretacionClima(scores.globalLevel), W - 28);
    doc.text(lineas, 14, afterDims + 6);

    // Pie
    doc.setFontSize(7.5);
    doc.setTextColor(150, 150, 150);
    doc.text("MINDTALENT · gerencia@mindtalentrh.com · Encuesta de Clima Laboral", 14, 285);
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: "#f0f4f8" }}>
      <div className="bg-white rounded-2xl shadow p-8 text-center max-w-md">
        ...
        <Link href="/dashboard" className="mt-4 inline-block text-sm underline" style={{ color: "#1a2035" }}>Volver al dashboard</Link>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen" style={{ background: "#f0f4f8" }}>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow p-6">
          <h1 className="text-xl font-bold mb-1" style={{ color: "#1a2035" }}>Informe Individual — Clima Laboral</h1>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 text-sm">
            {[
              { l: "Nombre",  v: rec.nombre  ?? "—" },
              { l: "Cargo",   v: rec.cargo   ?? "—" },
              { l: "Área",    v: rec.area    ?? "—" },
              { l: "Empresa", v: rec.empresa ?? "—" },
            ].map(({ l, v }) => (
              <div key={l}>
        ...
        <div className="rounded-2xl shadow p-6 text-center" style={{ background: "#1a2035" }}>
          ...
          <p className="text-white text-xs mt-3 opacity-70 max-w-xl mx-auto">{interpretacionClima(scores.globalLevel)}</p>
        </div>
          ...
          <div className="space-y-3">
            {scores.dimensions.map((d) => (
              <div key={d.code} className="flex items-center gap-4">
                <div className="w-44 text-sm font-medium text-gray-700 shrink-0">{d.label}</div>
                <div className="flex-1 bg-gray-100 rounded-full h-3">
                  <div
                    className="h-3 rounded-full"
                    style={{ width: `${(d.mean / 5) * 100}%`, background: getClimaLevelColor(d.level) }}
                  />
                </div>
                <div className="w-14 text-right text-sm font-bold" style={{ color: getClimaLevelColor(d.level) }}>
                  {d.mean.toFixed(2)}
                </div>
                <div className="w-28 text-xs font-semibold" style={{ color: getClimaLevelColor(d.level) }}>
                  {d.level}
                </div>
              </div>
            ))}
